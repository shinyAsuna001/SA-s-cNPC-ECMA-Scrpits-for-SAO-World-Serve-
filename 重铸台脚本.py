...
def interact(c):
    ...
    target_lore_num=0
    if item.getNbt().has('newitem')==False or item.isEmpty()==True:
    ...
    gui.addLabel(7, u"§8" + u"武器损毁概率:--",380, 275+22 , 29, 29, 16755200)
...
def customGuiButton(c):
    ...
    n=c.buttonId
    global ATK
    ATK=searchlore("ATK",lore[6],player)
    global ANG
    ANG=searchlore("ANG",lore[7],player)
    global END
    ...
    if c.buttonId > 9:
        ...
        gui.getComponent(7).setText(u"§8" + u"武器损毁概率: "+str(destroy_rate))
        gui.update(player)
    if c.buttonId == 9:
        count_lore=lore[0]
        if  u"§" in count_lore:    
        ...
        if colnum >= cost:
            ...
            gui.getComponent(7).setText(u"§8" + u"武器损毁概率:--")
            ...
            if random_product < success_rate:
                ...
                if readUpInfo(lore[target_lore_num])[0]==u'ATK':
                    global ATK
                    ATK=ATK-readUpInfo(lore[target_lore_num])[1]
                    lore[6]=u"§7⩺§l ATK :§c "+str(ATK)
                    item.setAttribute("minecraft:generic.attack_damage",ATK-1,0)
                elif readUpInfo(lore[target_lore_num])[0]==u'ANG':
                    global ANG
                    ANG=ANG-readUpInfo(lore[target_lore_num])[1]
                    lore[7]=u"§7⩺§l ANG :§3 "+str(ANG)
                    item.setAttribute("epicfight:armor_negation",ANG,0)
                ...
                for i in range(cycle):
                    if readUpInfo(lore[-i-1])[0]==u'ATK':
                        global ATK
                        ATK=ATK-readUpInfo(lore[-i-1])[1]
                        lore[6]=u"§7⩺§l ATK :§c "+str(ATK)
                    elif readUpInfo(lore[-i-1])[0]==u'ANG':
                        global ANG
                        ANG=ANG-readUpInfo(lore[-i-1])[1]
                        lore[7]=u"§7⩺§l ANG :§3 "+str(ANG)
                ...
                player.message(u"§7[§6SAO-World§7]§8[§a+§8]§7[§3重铸系统§7] §c武器在重铸中损坏了")
