...
min_DEF_level=0.5 #强化DEF的最低添加值
max_DEF_level=2 #强化DEF的最高添加值
min_MHP_level=1 #强化MHP的最低添加值
max_MHP_level=5 #强化MHP的最高添加值
...
def interact(c):
    ...
    if len(item.getDisplayName())>=3:
        ...
        return False
    if item.getNbt().has('IsArmor') and item.isEmpty()==False:
        ...
        global DEF
        DEF=searchlore("DEF",ilore[6],player)
        global MHP
        MHP=searchlore("MHP",ilore[7],player)
        global END
        ...
        gui.addLabel(3, u"§8防御方向", 470, 130 + 65+22, 100, 100, 16755200)
        gui.addButton(4, u"强化", 490 + 30, 175 + 15 +22, 50, 20).setHoverText(u"§7追加：§a"+str(min_DEF_level)+u"§7—§a"+str(max_DEF_level))
        gui.addLabel(5, u"§8生命方向", 470, 150 + 65+22, 100, 100, 16755200)
        gui.addButton(6, u"强化", 490 + 30, 195 + 15+22, 50, 20).setHoverText(u"§7追加：§a"+str(min_MHP_level)+u"§7—§a"+str(max_MHP_level))
...
def customGuiButton(c):
    ...
    DEF=searchlore("DEF",ilore[6],player)
    global MHP
    MHP=searchlore("MHP",ilore[9],player)
    global END
    END=searchlore("END",ilore[8],player)
    if c.buttonId == 4:
        randomnum=0
        ...
        if randomnum <= SuccessRate:
            randomlevel=round(random.uniform(min_DEF_level,max_DEF_level),1)
            new_DEF=DEF+float(randomlevel)
            new_lore[6]=u"§7⩺§l DEF :§c "+str(new_DEF)
            if not(is_lored):
                new_lore.append(u"§f=============§7[§6强化信息§7]§f=============")
            new_lore.append(u"§7[§a+§7] DEF -> §a+"+str(randomlevel))
            item.setLore(new_lore)
            item.setAttribute("minecraft:generic.armor",new_DEF,typeint)
    ...
    if c.buttonId == 6:
        ...
        if up_count>=max_count:
            ...
            if colnum >= cost:
                ...
                return False
        if randomnum <= SuccessRate:
            randomlevel=round(random.uniform(min_MHP_level,max_MHP_level),1)
            new_MHP=MHP+float(randomlevel)
            new_lore[9]=u"§7⩺§l MHP :§d "+str(new_MHP)
            if not(is_lored):
                new_lore.append(u"§f=============§7[§6强化信息§7]§f=============")
            new_lore.append(u"§7[§a+§7] MHP -> §a+"+str(randomlevel))
            item.setLore(new_lore)
            item.setAttribute("minecraft:generic.max_health",new_MHP,typeint)
            player.message(u"§7[§6SAO-World§7]§8[§a+§8]§7[§3强化系统§7] §7强化成功！属性提升"+str(randomlevel))
            player.playSound(SE1, 1, 1)
        else:
            item.setLore(new_lore)
...
def searchfornum(DEF,lore,player):    
    ...
    DEF=''
    for j in range(0,6):
        if str(DEF[j]) in numbers:
            DEF += DEF[j]
    if DEF[len(DEF)-1] not in numbers:
        DEF=DEF[:len(DEF)-1]
    DEF=float(DEF)
    return DEF
